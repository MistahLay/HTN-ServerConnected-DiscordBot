...
import { commands, commandsInfo, getCommands} from "./BotCommand";
...
client.on("messageCreate", async msg => {
    ...
    if(!(cmd.paramType.length === 0) && !(cmd.paramType.length <= args.length)) {
        msg.reply({embeds:[{
            title: "Command Error",
            color: Colors.Red,
            description: "Command is missing "+(cmd.paramType.length - args.length)+" arguments pls do ``c?help <command>`` to get more info about the command"
        }]})
        return;
    }
